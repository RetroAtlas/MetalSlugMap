...
- **Piece order is file order.** Within a section, the tilemaps matching the tallest piece height are the playfield, and they run left to right in the order they appear — Mission 1's eleven 256px-tall pieces lay end to end as one 3216px stage with an unbroken ground line. The shorter tilemaps are parallax and set dressing, drawn at their own scroll rates.
...
- Tilemaps and tile lists also appear **after** a file's leading TIM art, so a chunk walk that starts at offset 0 and stops at the first non-chunk misses most of them: scanning the whole file finds 290 tilemaps across 25 section files, against 128 from walking alone.
