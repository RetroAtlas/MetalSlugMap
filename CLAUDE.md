...
- How sections join into one scrolling mission is **still open** — each renders as its own piece. The remaining unidentified tables (`00 00 55 00`, the `pQES` sequences) and the per-section code overlays are where the ordering, scroll extents and object spawns must live.
