...
8. **Object placement** — in progress, by running the game rather than reading it. `tools/machine.py` boots the executable under an R3000A interpreter, answers the disc out of the image, and lets the game reach a stage the way it does with nobody playing: the title screen idles and starts its attract demo, and the only steering is which of the nine sub-stages that demo picks. The stage then loads its own files and creates its own objects. That already pays for itself as a check on the extraction — the file list a stage asks for is the game's own answer to which sections it plays in, and for mission 1 it matches, name for name and in order, what the streaming order above had inferred. Reading the objects back out needs the camera to move, and it does not yet.
